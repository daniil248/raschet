...
export const APP_VERSION = '0.59.293';
...
export const DEFAULTS = {
  ...
  panel:     () => ({
    ...
    marginMinPct: 2,
    marginMaxPct: 30,
    // Daisy-chain (шлейфовое подключение НКУ по входу):
    //   chainedFromId = id другого НКУ того же напряжения; текущий щит
    //   подключается по входу от того (без собственного автомата ввода,
    //   защита — общий upstream-автомат).
    // Следствие: все кабели между щитами в цепочке подбираются по
    // наибольшей суммарной нагрузке (один автомат защищает все).
    chainedFromId: null,
  ...
  // Junction Box (клеммная коробка / кабельный ящик).
  // N входов = N выходов: вход i идёт напрямую на выход i через клеммник.
  // В каждой цепи (channel) опционально стоит защитный аппарат
  // (автомат / предохранитель). Между входами возможны перемычки
  // (bridges) — но ТОЛЬКО до защитного аппарата.
  //
  // Влияние на подбор кабеля:
  //   - если на канале i стоит защита → отходящий кабель подбирается
  //     по этой защите (trip current breakerA);
  //   - если защиты нет → отходящий кабель должен выдерживать защитный
  //     аппарат вышестоящего шкафа (берётся по входной цепи).
  //
  // bridges: Array<[iA, iB]> — пары индексов входов, соединённых
  // перемычкой ДО защиты. Входы в перемычке делят ток пропорционально
  // (упрощённо — перераспределение не моделируем, но помечаем цепь
  // как «shared»).
  'junction-box': () => ({
    name: 'Клеммная коробка', comment: '',
    inputs: 2, outputs: 2,          // всегда равны (N-в → N-выход)
    channels: [                      // per-channel защита
      { hasProtection: false, protKind: 'breaker', breakerA: 0, fuseA: 0 },
      { hasProtection: false, protKind: 'breaker', breakerA: 0, fuseA: 0 },
    ],
    bridges: [],                     // [[iA,iB], ...] — перемычки входов до защиты
    ipRating: 'IP54',
    capacityA: 63,
  }),
  // ------- Новые типы -------
