/* =========================================================================
   GE Tools (Genesis Engineering Tools) — набор инженерно-управленческих модулей
   -------------------------------------------------------------------------
...
export const GLOBAL = {
  ...
  // v0.59.654: методика расчёта.
  // 'iec' — IEC 60364-5-52 (выбор сечения кабеля по таблицам ампасити IEC)
  // 'pue' — ПУЭ 7 (таблицы ПУЭ для кабеля)
  // 'rtm' — РТМ 36.18.32.4-92 (метод упорядоченных диаграмм Каялова —
  //          автоматический расчёт максимальной нагрузки через Ки и Кмакс
  //          по таблице приложения 2 РТМ; формулы: P_ср = Σ Ки × P_ном,
  //          n_э = (Σ P_ном)² / Σ (P_ном²), Кмакс = f(n_э, Ки.ср),
  //          P_макс = Кмакс × P_ср).
  //          v0.59.655: для расчёта кабеля при calcMethod='rtm' используется
  //          ПУЭ (см. js/methods/index.js — getMethod), т.к. РТМ — советский
  //          стандарт и логически ближе к ПУЭ, чем к IEC.
  // n._rtmMax всегда вычисляется (доступно для отчётов независимо от
  // режима), но _maxLoadKw перезаписывается на P_макс_РТМ только когда
  // calcMethod==='rtm'.
  calcMethod: 'iec',
  ...
  earthingSystem: 'TN-S',
  // v0.60.219 (по TODO из v0.60.205-206 — placeholder реализация). Режим
  // напряжения для расчёта тока «Макс»:
  //   • 'real'    — терминальное U с учётом ΔU% от источника. Идентичные
  //                 щиты в активном/резервном плече дают РАЗНЫЕ числа
  //                 Макс (в активном плече ΔU больше → меньше U → больше I).
  //   • 'nominal' — Unom (vLL/vLN) без ΔU. Идентичные щиты дают одинаковые
  //                 числа. Удобно для сравнения и проверки.
  // Применяется в nodeCalcVoltageEff() (electrical.js); recalc.js использует
  // её только для _maxLoadA. Cable sizing и _loadA продолжают использовать
  // Unom (nodeCalcVoltage), потому что подбор кабеля — по nominal-параметрам.
  calcVoltageMode: 'real',
  // v0.60.232 (по запросу Пользователя 2026-05-05 «давай сделаем вариант В,
  // пусть пользователь решает, как ему считать»): база для агрегации «Макс»
  // на уровне щитов/PDM. Не путать с расчётом тока для линии К ОДНОМУ
  // потребителю — там всегда Pном (consumerNominalCurrent).
  //   • 'nameplate'  — Σ P_ном downstream без учёта Ки. Консервативно.
  //                    Pуст.
  //   • 'calculated' — Σ P_ном × Ки = Pрасч. Соответствует ПУЭ 1.3.13 /
  //                    IEC 60364-3 §311.1 (учёт diversity для агрегатов).
  //                    По умолчанию (v0.60.241).
  // Применяется в:
  //   - maxDownstreamLoad → _bfsDownstreamWithActiveTies
  //   - sibling-clamp в recalc.js (v0.60.229)
  //   - _sectionedContainerAgg (многосекционные щиты)
  // v0.60.241 (по уточнению Пользователя 2026-05-05 «вся правда в том что
  // максимум и должен отображать максимальные параметры в любом режиме» /
  // «то есть 97,3 кВт а не 60,4 или 65,8»): default переключён с 'nameplate'
  // на 'calculated'. Т.к. Макс = реальный max-scenario flow ≈ Pрасч × union,
  // а не nameplate-сумма (это не достижимая paper-цифра). Согласуется с
  // ПУЭ 1.3.13 / IEC 60364-3 §311.1 — стандарт для агрегаторов.
  panelMaxBasis: 'calculated',
...
// v0.60.256 (по запросу Пользователя 2026-05-06): добавлен 500 А — есть в IEC 60947-2
// (Schneider ComPacT NSX500/NS630 в одном корпусе с rating plug 500 A; ABB Tmax XT
// / Tmax T5 with 500 A trip; Siemens 3VA in 500 A range). Также 315 А — стандартный
// промежуточный шаг (250→315→400 в gG-fuse и MCCB-trip plugs).
export const BREAKER_SERIES = [6, 10, 13, 16, 20, 25, 32, 40, 50, 63, 80, 100, 125, 160, 200, 250, 315, 400, 500, 630, 800, 1000, 1250, 1600, 2000, 2500, 3200, 4000, 5000, 6300];
...
export const DEFAULTS = {
  ...
  // v0.59.815: контейнер потребителей на СХЕМЕ — организационная обёртка
  // над набором независимых consumer-узлов и placeholder-спецификаций.
  //
  // СОСУЩЕСТВУЕТ с group-consumer (consumer с count>1):
  //   • Group consumer = однородная масса («10 лампочек × 50 Вт»),
  //     идентичность членов не важна → один узел.
  //   • Consumer container = идентичность ВАЖНА (SR01..SR08) → wrapper
  //     с linked-slots, каждый — свой consumer-узел.
  //   • Слот контейнера может ссылаться на group-consumer узел
  //     (один слот = 10 лампочек × 50 Вт), чтобы не плодить 10 узлов.
  //
  // ВАЖНО: это про схематический редактор. Концепция стоек в модуле
  // Технолог-ЦОД — отдельный концепт (там действительно шаблон
  // требований с maxCount/маской/template params), синхронизация с
  // ним — отдельной задачей.
  //
  // Идея (по требованиям пользователя 2026-04-30):
  //   • Контейнер сам НЕ потребитель. Нет demandKw/cosPhi/phase
  //     собственных. Не учитывается в реестре как consumer, не считается
  //     потребителем в recalc/POR/BOM напрямую.
  //   • Каждый «slot» внутри — независимая сущность:
  //       - {kind:'linked', nodeId} — ссылка на реальный consumer-узел
  //         (свои параметры, своя карточка в реестре, скрыт с холста
  //         через containerId);
  //       - {kind:'placeholder', demandKw, cosPhi, phase, voltage, subtype, name?}
  //         — анонимная спецификация для «слота-заглушки» (планирование
  //         нагрузок до выбора конкретной модели потребителя).
  //   • Рендер: stacked card как у старой группы. Обозначение/имя
  //     контейнера = от linked-потребителя с самым младшим tag'ом
  //     (placeholders без tag не участвуют).
  //   • Расчёты: recalc/electrical раскрывают slots — каждый linked
  //     даёт вклад через свои params (см. consumerTotalDemandKw),
  //     каждый placeholder — через инлайн-спеку.
  //   • Графовые порты: контейнер принимает вход питания (родитель =
  //     шкаф/PDU) как один узел в графе. Линки наружу не предусмотрены
  //     (потребители-листья).
  'consumer-container': () => ({
    name: 'Контейнер потребителей', comment: '',
    inputs: 1, outputs: 0,
    inputSide: 'top',
