    ...
    if (!isUtilityInfeed && (c._cableSize || c._busbarNom || c._cableIz)) {
      ...
      if (GLOBAL.showHelp !== false && c._cableSize) {
        ...
        if (isGroupBreakers) {
          ...
          // v0.59.676: добавлена строка про «Свободно» — пользователь
          // попросил «приписку (автомат или кабель можно написать только
          // в примечаниях к расчету)».
          const _toN = state.nodes.get(c.to.nodeId);
          ...
          const _vdNote = (() => {
            if (!Number.isFinite(c._cableVdropPct)) return '';
            const _vd = c._cableVdropPct;
            const _norm = Number(c.maxVdropPct) || Number(GLOBAL.maxVdropPct) || 5;
            const _ok = _vd <= _norm;
          ...
          // фактических цифр. Теперь — оцениваем и показываем ✓/⛔.
          const _ruleCableOk = (Number(_IperLine) || 0) <= (Number(Iz) || 0);
          const _ruleBrkOk = brkRef ? (Number(brkRef) || 0) <= (Number(Iz) || 0) : true;
          const _ruleColor = (_ruleCableOk && _ruleBrkOk) ? '#15803d' : '#b91c1c';
          ...
          const _IzCheck = (par > 1 && Number.isFinite(IzTotal)) ? IzTotal : Iz;
          const _ruleCableOk = (Number(Iraw) || 0) <= (Number(_IzCheck) || 0);
          const _ruleBrkOk = effectiveBrkIn ? (Number(effectiveBrkIn) || 0) <= (Number(_IzCheck) || 0) : true;
          const _ruleColor = (_ruleCableOk && _ruleBrkOk) ? '#15803d' : '#b91c1c';
          const _ruleIcon = (_ruleCableOk && _ruleBrkOk) ? '✓' : '⛔';
