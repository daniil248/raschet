...
export function openUpsParamsModal(n) {
  ...
  // v0.59.699: helpIcon на заголовках секций SBS и параллельной работы.
  const _sbsTip = 'Статический байпас (SBS) — электронный (тиристорный) обходной путь вокруг инвертора. При перегрузе или отказе инвертора нагрузка автоматически переключается на сеть напрямую без прерывания. Принудительная активация SBS, авто-переход по перегрузу и порог — в модалке «🔌 Управление ИБП».';
  h.push(`<h4 style="margin:16px 0 8px">Статический байпас (SBS)${helpIcon(_sbsTip)}</h4>`);
  h.push(`<div class="field check"><input type="checkbox" id="up-bypass"${n.staticBypass !== false ? ' checked' : ''}><label>Байпас разрешён (допускается переход на SBS)${helpIcon('Если выключено — ИБП не будет переходить на SBS даже при перегрузе. Применяется когда сеть нестабильна и переход на байпас опасен (нагрузка получит «грязное» напряжение). Для критичных ЦОД отключают этот флаг и работают только через инвертор + АКБ.')}</label></div>`);
