...
function recalc() {
  ...
  // v0.60.240 (по уточнению Пользователя 2026-05-05 «если я для кондиционера
  // поставлю питание с 2 вводов одновременно, то мощности на панелях будут
  // равны, а если питание по умолчанию только с панели номер 1 то
  // различаются, но это не правильно, потому что если отключить панель
  // номер 1, то вся нагрузка прилетит на панель номер 2»):
  // ВОЗВРАЩАЕМ sibling-clamp с двумя защитами от false-positives v0.60.229:
  //   1. Skip <code>_isInternalIntegrated</code> conns (фабричные шины
  //      внутри Kehua MR33 — иначе walk от AC возвращался в UPS-parent
  //      и собирал ВСЮ нагрузку UPS1).
  //   2. 50%-threshold (Jaccard): sibling-ы только когда ≥50% общих
  //      consumer-ов меньшего из двух sets. АГПТ/слаботочка cross-feed
  //      (1 общий из 7 — 14%) НЕ делает AC ↔ IT siblings.
  // С этими защитами:
  //   • IT1+IT2 (≥80% общих consumer-ов, отличается только L7) — siblings,
  //     Макс выравнивается через union (включая «недоkonctructed» L7).
  //     Это решает «если отключить панель 1, нагрузка прилетит на панель 2».
  //   • AC — отдельно, ~7-8 кВт реальной нагрузки.
  if (true) {
