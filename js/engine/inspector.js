...
export function openUpsParamsModal(n) {
  ...
  if (applyBtn) applyBtn.onclick = () => {
    ...
    if (n.upsType === 'modular') {
      ...
      // Пересчитываем capacityKw из N × P
...
// Показывает текущие потоки (ток каждого автомата), состояние автоматов с toggle,
// и структурную схему ИБП с визуализацией активных ветвей.
...
function _renderUpsControlBody(n) {
  ...
  // Расчёт токов для каждой ветви (упрощённо)
  const U = nodeVoltage(n);
...
// Структурная схема ИБП: вход → QF1 → выпрямитель → инвертор → QF3 → выход
//                              ↓
//                           батарея ← QB
//                       QF2 → статбайпас → (обход к QF3)
// Активные ветви подсвечиваются цветом.
function _upsStructSvg(n, flows) {
  ...
  // Цвет каждой ветви в зависимости от режима
  ...
  // QF4 — механический ручной байпас (обход ИБП целиком)
...
function _svgBreaker(cx, cy, label, color, present, orient = 'right') {
  if (!present) {
    // отсутствующий — тонкий серый контур
