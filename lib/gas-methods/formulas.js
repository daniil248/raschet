 ...
 *  Pa²−Pb²[бар²] = 48600 · dRel · L_km · Qh^1.82 / D_mm^4.82
 *  ВНИМАНИЕ конвенция: константа 48600 определена для L в КМ и P в
 *  бар (в отличие от linéaire: 23200, L в м, P в мбар). Проверено
 *  численно — при ΔP«P quadratique сходится с linéaire (v0.60.588).
 *  L подаётся в МЕТРАХ → делим на 1000. → возвращаем [Па²]. */
export function renouardQuadraticDSq(dRel, L, D_mm, Qh) {
  if (D_mm <= 0) return 0;
  const L_km = (Number(L) || 0) / 1000;
  const dSqBar2 = 48600 * (Number(dRel) || 0) * L_km
    * Math.pow(Math.max(Qh, 0), 1.82) / Math.pow(D_mm, 4.82);
