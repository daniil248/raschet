...
> **Статус:** v0.60.522 (2026-05-16). Текущий приоритет — **архитектурная
> модуляризация**: Фаза 0 ✅ (boundary-lint+CI+baseline), Фаза 1 ✅
> (15 manifest.json + gen-modules-json, паритет, zero-risk), Фаза 2 в
> работе — burndown allowlist (#1 ✅ psychrometrics-core→shared/calc;
> #2 ✅ meteo/util→shared/meteo-util — гашены cooling+service).
...
- [~] X.1.3 Упрочнение границ + гашение allowlist (Фаза 2).
  Сделано: #1 psychrometrics-core → `shared/calc/` (shim);
  #2 meteo/util → `shared/meteo-util.js` (shim; cooling+service
  гашены). Осталось: cooling→meteo/charts, service→cooling/calc,
  js/main→reports-seed, battery-seed→battery; затем
  `projects/project.js` (15+ чужих LS) и `*-bridge.js` → через
  project-storage/configuration-catalog helpers.
