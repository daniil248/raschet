...
> **Статус:** v0.60.530 (2026-05-16). Текущий приоритет — **архитектурная
...
> **реестр полон — modules.json v1.3.1, 28 UI + 1 calc-lib**
> (UNREGISTERED 0, PARITY 0). `dgu-config`→`genset-config`
> (англоязычный аналог; LS/протокол сохранены, данные не тронуты).
> Осталось (R2, advisory): projects/project.js (15+ чужих LS) +
> `*-bridge.js`; синк `requires` (UNDECLARED ≈32, Фаза 1).
...
- [~] X.1.4 Структура-канон + calc/ui-нормализация. memory:module_separation
  ...
  Осталось: синк `requires` (UNDECLARED ≈32, мост-зависимости, по
  модулю), дробление монолитов (calc→ui, shim).
