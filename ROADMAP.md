...
> **Статус:** v0.60.651 (2026-05-17). Текущий приоритет — **редизайн
> модуля отчётов (X.6.3): единый flow-документ** (R1 модель — в работе;
> устраняет наложение структурных зон поверх контента; discovery
> зафиксирован). Архитектурная
...
- [~] X.6.3 Кастомные сохраняемые шаблоны документов (модуль отчётов). `arch:Фаза 29`
  ...
  - **РЕДИЗАЙН (discovery 2026-05-17): единый flow-документ.** Проблема:
    ...
    превью), сразу. Фазы:
    - [~] **R1** модель `flow[]`/`floating[]` + `migrateToFlow()` +
      `effectiveFlow()` + типы структурных блоков (`template.js`,
      без потребителей — cache-safe).
    - [ ] **R2** рендереры (preview/pdf/docx) на `effectiveFlow`,
      резервирование структурных блоков, floating = единственный
      absolute; absolute-overlay-путь структуры выключается.
    - [ ] **R3** редактор «Структура документа» (flow-список: drag/
      show-hide/props/+структурный блок; вкладки Лист/Колонтитулы/
      Плавающий слой/Стили; объединяет Зоны+Разделы; WYSIWYG=поток).
    - [ ] **R4** быстрая правка из превью экспорта (кнопка «⚙ Шаблон»).
    - [ ] **R5** миграция отчётов (ТЗ/КП) + комплектные встроенные
      шаблоны на flow (поглощает обогащение сидов).
    - [ ] **R6** чистка legacy absolute-overlay-пути (D6;
      `migrateToFlow` на load держит старые шаблоны).
