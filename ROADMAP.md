...
> **Статус:** v0.60.531 (2026-05-16). Текущий приоритет — **архитектурная
...
> **реестр полон + manifest-контракт честен — modules.json v1.3.2**
> (audit-manifest зелёный: UNDECLARED 0 / PARITY 0 / UNREGISTERED 0).
> `dgu-config`→`genset-config` (LS/протокол сохранены).
...
- [~] X.1.4 Структура-канон + calc/ui-нормализация. memory:module_separation
  ...
  полностью зелёный (UNDECLARED 0). Осталось: дробление монолитов
  (calc→ui, shim, по 1 модулю).
