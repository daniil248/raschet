...
> **Статус:** v0.60.556 (2026-05-17). Текущий приоритет — **архитектурная
...
- [~] X.1.3 Упрочнение границ + гашение allowlist (Фаза 2).
  ...
  Префикс-сканы R2 ✅ (v0.60.556): helper `projectPrefix`/
  `projectModulePrefix` (через APP_NS) + 6 префикс-сканов
  (cooling/meteo/service/scs-design×2/scs-config) + 3 остаточных
  full-key в scs-config. Итог: project.js 36 + батч 18 + финал 9 =
  **63 сырых доступа погашены**, raw `raschet.project.${` в модулях
  не осталось.
  Осталось (R2, advisory): упрочнение `*-bridge.js` (санкц.
  адаптеры) → через project-storage/configuration-catalog helpers.
