...
> **Статус:** v0.60.542 (2026-05-16). Текущий приоритет — **архитектурная
...
- [ ] X.5.0 **Importmap-развязка путей** (enabler физ. реструктуризации
  ...
  по 1 модулю: конвертация `../shared/`,`../../shared/`,`../js/engine/`
  → bare `shared/`/`engine/` (cooling ✅). Затем **отдельная
  нормализация не-JS ссылок** (27 CSS-link / 77 script-src / 8
  import.meta.url / 2 fetch — importmap их НЕ покрывает), затем
  группировка в `apps/`+`lib/`+`platform/`.
