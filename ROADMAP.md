...
> **Статус:** v0.60.535 (2026-05-16). Текущий приоритет — **архитектурная
...
> `dgu-config`→`genset-config`. Дробление монолитов: calc/ui выделен в
> `transformer-config/calc/tx-select.js` (пилот) + `ups-config/calc/ups-sizing.js`
> + `rack-config/calc/rack-power.js` + `scs-config/calc/rack-slots.js`.
...
- [~] X.1.4 Структура-канон + calc/ui-нормализация. memory:module_separation
  ...
  Осталось по 1 модулю: scs-design (~5.2k), tech-workspace (5.7k).
