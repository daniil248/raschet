...
> **Статус:** v0.60.577 (2026-05-17). Текущий приоритет — **архитектурная
...
- [~] X.4.2 Cross-discipline schemes (узел в неск. дисциплинах). `arch:47.4.3`
  ...
  без import lib (calcLib = bare-строка для потребителя). Осталось:
  модель узла в неск. дисциплинах в CORE (serialization-миграция) +
  UI — отдельной одобренной под-фазой (CORE-риск).
