...
> **Статус:** v0.60.564 (2026-05-17). Текущий приоритет — **архитектурная
...
- [~] **1.7** Все отчёты только через `reports/` (blocks API).
  Сделано: Service export-offer ✅ (v0.60.26 через shared/report/
  blocks); SCS-BOM/TW-PUE — прямого HTML нет (не анти-паттерн).
  Остаток (genuine direct-HTML `window.open('').document.write`):
  pdu-config ✅ (v0.60.562), logistics ✅ (v0.60.563),
  battery ✅ (v0.60.564 — делегирует общий buildBatteryReportBlocks);
  осталось `ups-config/ups-config.js:2221` (визуальный diff-парность).
