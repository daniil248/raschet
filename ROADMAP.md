...
> **Статус:** v0.60.539 (2026-05-16). Текущий приоритет — **архитектурная
...
- [ ] X.5.0 **Importmap-развязка путей** (enabler физ. реструктуризации
  без build). 🧪 Прототип на `cooling/` ✅ (v0.60.539): bare-спецификаторы
  `shared/`/`engine/` + per-HTML importmap, папка не двигалась. Далее по
  факту verify на проде: либо раскат на ~25 entry-HTML + нормализация
  не-JS ссылок (27 CSS-link / 77 script-src / 8 import.meta.url / 2 fetch),
  потом группировка в `apps/`+`lib/`+`platform/`; либо откат прототипа.
