...
> **Статус:** v0.60.540 (2026-05-16). Текущий приоритет — **архитектурная
...
- [ ] X.5.0 **Importmap-развязка путей** (enabler физ. реструктуризации
  ...
  модулям. Механизм пригоден. Следующий шаг (по решению): раскат на
  ~25 entry-HTML + **отдельная нормализация не-JS ссылок** (27 CSS-link
  / 77 script-src / 8 import.meta.url / 2 fetch — importmap их НЕ
  покрывает), затем группировка в `apps/`+`lib/`+`platform/`. Либо
  откат прототипа (1 модуль, обратимо).
