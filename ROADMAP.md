...
> **Статус:** v0.60.543 (2026-05-16). Текущий приоритет — **архитектурная
...
- [ ] X.5.0 **Importmap-развязка путей** (enabler физ. реструктуризации
  ...
  файла. Шаг 3 — **нормализация не-JS ссылок** (27 CSS-link / 77
  script-src / 8 import.meta.url / 2 fetch — importmap их НЕ
  покрывает), затем физический переезд папок в
  `apps/`+`lib/`+`platform/` (modules.json/hub/inspector/URL).
