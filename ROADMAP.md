...
> **Статус:** v0.60.675 (2026-05-17). Модуль отчётов (X.6.3)
> **завершён ПОЛНОСТЬЮ** (R1–R6 + RR1-3 + DS1-4): flow-документ
> (наложение устранено), редактор на flow (WYSIWYG=PDF), правка из
> превью, общий `composeReport` для ВСЕХ отчётов, Word-style
> разделы (обложка + sectionBreak, перем. ориентация preview/PDF/
> DOCX), legacy absolute-overlay убран. Verified e2e. Архитектурная
...
- [x] X.6.3 Кастомные сохраняемые шаблоны документов (модуль отчётов) — flow-редизайн завершён ПОЛНОСТЬЮ (R1–R6 + RR1-3 + DS1-4 Word-разделы). `arch:Фаза 29`
