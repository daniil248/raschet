...
> **Статус:** v0.60.532 (2026-05-16). Текущий приоритет — **архитектурная
...
> `dgu-config`→`genset-config`. Дробление монолитов начато:
> `transformer-config/calc/tx-select.js` (пилот calc/ui).
...
- [~] X.1.4 Структура-канон + calc/ui-нормализация. memory:module_separation
  ...
  полностью зелёный (UNDECLARED 0). Дробление монолитов: пилот
  `transformer-config/calc/tx-select.js` ✅ (v0.60.532, паттерн
  доказан). Осталось по 1 модулю: ups-config (2.2k), rack-config
  (2.7k), scs-config/scs-design (~5.2k), tech-workspace (5.7k).
