...
> **Статус:** v0.60.527 (2026-05-16). Текущий приоритет — **архитектурная
...
> (код не тронут). Осталось (R2, advisory): projects/project.js
> (15+ чужих LS) + упрочнение `*-bridge.js` через project-storage;
> синк `requires` + регистрация 13 ui/calc-lib (Фаза 1).
...
- [~] X.1.4 Структура-канон + calc/ui-нормализация. memory:module_separation
  ...
  + CI non-blocking. Осталось: синк `requires` (Фаза 1), регистрация
  13 ui + calc-lib, дробление монолитов по 1 (calc→ui, shim).
