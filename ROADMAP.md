...
- [x] **1.22.1** Вкладка «📋 Общее» (v0.58.47) — первая по порядку:
  - обозначение (tag), полное обозначение (effectiveTag),
  - имя, инв.№ / паспорт (`n.assetId`),
  - UUID (read-only),
  - блок «Модель изделия»: поле `n.modelRef` + кнопка
    «🔧 Конфигурировать…» с переходом в соответствующий модуль
    (transformer-config / panel-config / mv-config / ups-config /
    rack-config / sks-config — по типу элемента),
  - комментарий.
- [x] Электрика-вкладка очищена: tag/name/комментарии перенесены
  в «Общее» (дублирования нет).
- [ ] **1.22.2** Modal-параметры (open*ParamsModal в panel/source/ups)
  тоже получают вкладку «Общее» — сейчас остаётся только в
  sidebar-инспекторе.
- [ ] **1.22.3** Единый CSS-стиль всех `.tp-panel`: одинаковый
  `.inspector-section h4`, отступы, левые цветные полоски только
  на системных вкладках.
- [ ] **1.22.4** `n.modelRef` становится обязательным «указателем» в
  каталог выбранного модуля. Это создаёт связь между элементом
  схемы и конкретным SKU — основа для BOM с точными артикулами.
...
- [ ] **1.23.4** Аксессуары:
  ...
  - основание / пол (с кабельными вводами — кол-во, тип: щётки/
    заглушки/прокладки),
  - цоколь (ножки / ролики).
- [ ] **1.23.5** Кабельные вводы (top/bottom/side): количество,
  диаметр, тип (щётка / заглушка / PG).
- [ ] **1.23.6** Заглушки на пустые юниты: кол-во по умолчанию =
  (`totalU − занятоU`), можно override. Тип: `1U глухая / 1U
  перфорация / 2U`.
- [ ] **1.23.7** PDU (блоки распределения питания):
  - количество PDU,
...
- [ ] **1.23.8** Карта юнитов: визуальная 2D-раскладка
  `frontView`/`rearView` — какие U заняты оборудованием (см. 1.24),
  какие — заглушками.
- [ ] **1.23.9** BOM на заказ: таблица со всеми позициями (корпус,
  двери, боковины, пол, кабвводы, PDU, заглушки). Экспорт CSV/PDF.
- [ ] **1.23.10** Сохранение конфигурации как шаблон:
  `project.rackTemplates[id]`. Применение шаблона к другим
  `rack`-карточкам одним кликом.
