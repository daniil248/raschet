...
> **Статус:** v0.60.552 (2026-05-17). Текущий приоритет — **архитектурная
...
- [~] X.1.3 Упрочнение границ + гашение allowlist (Фаза 2).
  ...
  Осталось (R2, advisory): прочие сырые читатели (projects.js,
  js/main.js, tech-workspace, scs-design, rack-sidebar и др.) +
...
- [~] X.5.2 project-storage как интерфейс (LS/HTTP/IDB/**FS** транспорты). `arch:17.2,34`
  ...
  - [ ] **FS-транспорт (запрос пользователя):** настройка «папка на
    ...
    (`showDirectoryPicker({startIn:'home'})`, persist directory
    handle в IDB, permission re-grant), запись данных проекта в
    выбранную папку вместо/вместе с LS. По умолчанию — домашняя
    папка (picker `startIn:'home'`; тихий доступ к ФС браузер не
    даёт — только через явный выбор). Транспорт за интерфейсом
    project-storage (load/save/list/remove), фолбэк на LS если API
    нет/не выбрана папка. UI — в global-settings.
