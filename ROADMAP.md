...
> **Статус:** v0.60.662 (2026-05-17). Редизайн модуля отчётов
> (X.6.3) **завершён**: единый flow-документ — наложение структурных
> зон поверх контента устранено (verified e2e), редактор перестроен
> на flow (WYSIWYG=PDF), правка из превью, комплектные сиды; R6
> (чистка legacy) опц. Архитектурная
...
- [x] X.6.3 Кастомные сохраняемые шаблоны документов (модуль отчётов) — flow-редизайн завершён (R1–R5, R6 опц.). `arch:Фаза 29`
  ...
  - **РЕДИЗАЙН (discovery 2026-05-17): единый flow-документ.** Проблема:
    ...
    - [ ] **R6** (опц., низкий приоритет) чистка `migrateLegacyToOverlays`
      в createTemplate (избыточен при flow); старый canvas-редактор
      уже удалён, дубль-наложение закрыто R5c. Функц. пробела нет.
