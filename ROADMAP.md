...
> **Статус:** v0.60.523 (2026-05-16). Текущий приоритет — **архитектурная
> модуляризация**: Фаза 0 ✅, Фаза 1 ✅, Фаза 2 в работе — burndown
> allowlist: #1 psychrometrics-core, #2 meteo/util, #3 CSV-хелперы →
> SHARED (cooling→meteo полностью устранён). Осталено: service→
> cooling/calc, js/main→reports-seed, battery-seed→battery.
...
- [~] X.1.3 Упрочнение границ + гашение allowlist (Фаза 2).
  Сделано: #1 psychrometrics-core → `shared/calc/`; #2 meteo/util →
  `shared/meteo-util.js`; #3 CSV-хелперы → `shared/meteo-util.js`
  (cooling→meteo полностью устранён, все со shim). Осталось:
  service→cooling/calc/fc-summary, js/main→reports-seed, battery-seed
  →battery; затем `projects/project.js` (15+ чужих LS) и
  `*-bridge.js` → через project-storage/configuration-catalog.
