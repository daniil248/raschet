...
>
> **Статус:** v0.60.757 (2026-05-18). Платформа + предметные модули.
...
**🔲 Открыто**
- [~] **8.0 Под-проект → ВАРИАНТ (модель + единый picker).** Решение
  Пользователя 2026-05-18: «под-проект» = на самом деле ВАРИАНТ внутри
  проекта (как подбор холода/ИБП: выбранный/резервный), НЕ отдельный
  проект, не называть проектом. Сделано (v0.60.751–753): фикс
  рассогласования namespace scs-design↔scs-config (`?project=`-шов);
  релейбл UI «под-проект/СКС-проект/мини-проект/подпроект» → «вариант»
  ...
  **Рефактор (по решению Пользователя, поэтапно с verify):**
  ...
  карточка бейдж+кнопки; аддитивно, 0 миграции). 8.0-B variant-aware
  единый picker ✅ (v0.60.756: секция «Варианты проекта» в шапке-
  поповере с ★/🔁 + переключением; согласовано с v0.60.349 —
  сгруппировано отдельно, общий список full-only). Отложено:
  8.0-C опц. слияние namespace вариантов внутрь проекта + миграция
  существующих sketch (риск, только по явному go Пользователя).
