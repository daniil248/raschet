...
> **Статус:** v0.60.545 (2026-05-16). Текущий приоритет — **архитектурная
...
- [ ] X.5.0 **Importmap-развязка путей** (enabler физ. реструктуризации
  ...
  Поведение идентично; будущий move = правка ОДНОЙ карты. Осталось:
  depth-fix не-JS на entry-HTML (CSS-link / script-src / importmap-
  адреса) + 2 лаунчера (hub/modules) + физический переезд папок в
  `apps/`+`lib/`+`platform/` + modules.json.
