...
- [ ] **1.21.2** Расширение связей — произвольные концы:
  ...
  - Файлы: `engine/interaction.js` (drag-handles на from-end),
    ...
    концов).
- [ ] **1.21.3** ИБП — байпасный вход:
  - Новое поле `n.bypassInputs: 0 | 1` (отдельный порт).
  - При работе через байпас: поток идёт мимо инвертора, КПД=100%,
    батарея не используется, зарядный ток = 0.
  - Активируется: основные входы обесточены И батарея разряжена, или
    вручную через `n.forceBypass`.
  - Инспектор: отдельная секция «Байпас» с выбором auto/force.
  - На плашке ИБП: индикатор режима — нормальный / батарея / байпас.
  - Файлы: `engine/inspector/ups.js`, `engine/recalc.js` (UPS power-
    flow ветка), `engine/render.js` (badge режима).
