...
> **Статус:** v0.60.525 (2026-05-16). Текущий приоритет — **архитектурная
> модуляризация**: Фаза 0 ✅, Фаза 1 ✅, Фаза 2 в работе — burndown
> allowlist (7→1): #1 psychrometrics-core, #2 meteo/util, #3 CSV,
> #4 service→shared/money, #5 battery-catalog→shared. Осталось:
> js/main→reports-seed; затем projects/project.js + bridges (R2).
...
| ups/battery/panel/mv/transformer/dgu/pdu/cable | стабильны; ручные габариты/паспорт ✅ (v0.60.514-519); residual — архив |
...
- [~] X.1.3 Упрочнение границ + гашение allowlist (Фаза 2).
  Сделано: #1 psychrometrics-core; #2 meteo/util; #3 CSV-хелперы →
  `shared/` (cooling→meteo полностью устранён); #4 service→
  `shared/money` (fc-summary дублировал money байт-в-байт; без shim);
  #5 каталог АКБ → `shared/battery-catalog.js` (зеркало ups-catalog;
  старый путь — shim; battery-seed→module устранён).
  Осталось: js/main→reports/templates-seed (R1, последняя запись);
  затем `projects/project.js` (15+ чужих LS) и `*-bridge.js` →
  через project-storage/configuration-catalog helpers.
- [ ] X.1.4 calc/ui-нормализация по 1 модулю (shim). memory:module_separation
