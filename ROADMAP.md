...
- [ ] 11.6 — гидравлический расчёт трубопровода: падение давления
      по Darcy-Weisbach / Benedict-Webb-Rubin (для смесей с фазовым
      переходом при выпуске хим. агентов).
- [ ] 11.7 — 3D-конфигуратор трубопровода на three.js: размещение
      форсунок по плану помещения, автоматическая трассировка
      магистрали с учётом колен и тройников, BOM с длинами.
- [ ] 11.8 — интеграция с mdc-config: взять объём IT-модулей из
      конфигуратора ЦОД, автоматически рассчитать АГПТ, вернуть
      BOM в главную спецификацию «Объём поставки».
- [ ] 11.9 — валидационные тесты против примеров в СП РК Приложение Г
      и NFPA 2001 Annex A.
...
- [ ] 12.1 — Модуль `psychrometrics/` (ID-диаграмма Молье-Рамзина /
