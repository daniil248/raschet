...
- [~] **1.24.3** Размещение в стойке: сейчас — через инпут U и
  автопоиск свободной области + авто-укладка. Конфликты подсвечиваются
  красным (перекрытия, наезд на «занятые» юниты rack-config, выход за
  границы). Полноценный drag-n-drop — следующая итерация.
- [~] **1.24.4** Привязка питания: поля `pduFeed` (A/B/…) и
  `pduOutlet` (№ розетки) на каждом устройстве. Предупреждение,
  если устройство с powerW>0 не привязано. Интеграция со списком
  розеток PDU из rack-config (1.23.7) и hard-check перегруза — далее.
