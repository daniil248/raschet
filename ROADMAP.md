...
>
> **Статус:** v0.60.759 (2026-05-18). Платформа + предметные модули.
...
- [ ] **6.2** Coordinated revisions (согласование разделов между
  дисциплинами) — под-фаза X.4.4. `arch:2.X.2,2.X.4`
...
**🔲 Открыто**
- [~] **8.0 Модель «Проект › Конфигурация › Вариант».** Discovery
  ...
  - **8.0-D FR1** аддитивное `entityKind` (object|discipline) +
    хелперы `entityKindOf` + инференс legacy (cache-safe: экспорты
    отдельным деплоем).
  - **8.0-E FR2** условный рендер карточки (`projects/project.js`):
    object→полная, discipline→слим (Сводка+Оборуд./Экономика/
    Проверки/Варианты+Управление дисциплины; скрыть категорию/
    локацию/реквизиты/ГИП/согласование/команду/сводный BOM/«Модули
    и связи»).
  - **8.0-F FR3** объект-props: конфигурация read-only от родителя,
    1-дисц.проект — свои мини-поля (шифр/заказчик/адрес).
  - **8.0-G FR4/FR5** создание с явным выбором типа + бейджи типа,
    добивка терминологии.
  - **8.0-H FR6** идемпотентный type-инференс существующих sketch
    (sketch+parent ⇒ конфигурация) — без переноса данных; verify
    «СКС» 240 связей в слим-карточке.
