...
> Ф-A…Ф-G ЗАВЕРШЕНЫ (Вариант I полностью) ✅. Модуль отчётов (X.6.3)
...
- [ ] X.4.5 **Архитектурные дополнения Пользователя (2026-05-17)** —
  ...
  - [~] X.4.5.3 D3: Конструктор — ОДИН app, единый интерфейс для
    ...
    Ф-E деривация ✅ (v0.60.731): deriveRegistry(pid) — read-only
    проекция RegistryObject[] из rack-config.instances.v1 +
    порт-инференс (PDU/нагрузка→power→электрика; СКС contents/
    links→data→СКС) + дисципл-срезы electrical/mechanical. Чистое
    чтение (источники остаются владельцами, не мутирует/не пишет),
    defensive guards, 0 потребителей cache-safe. Дальше Ф-E wiring
    (снимок в Конструктор/tech-workspace UI) → Ф-F → Ф-G.
    Ф-E wiring #1 ✅ (v0.60.732): tech-workspace sidebar +секция
    «📋 Реестр объектов» (deriveRegistry порт-driven: ⚡power→
    электрика, 🔌data→СКС). Первый реальный потребитель store
    (cache-safe: shared в 729–731, 2 деплоя). Fail-soft (try/catch
    IIFE, не ломает rail). Технолог-ГИП координатор (memory D7).
    Дальше Ф-F (СКС в оболочку) → Ф-G (агрегация BOM).
    Ф-F контракт-задел ✅ (v0.60.733): спека §5.1 — таблица «оболочка
    vs движок СКС» (SHELL-слои которые scs-* принимают vs движок
    «План зала»/владение данными которые остаются) + порядок код-
    инкрементов Ф-F1 (disc-context бар) → Ф-F2 (sidebar-аккордеон)
    → Ф-F3 (header/project-context/role-gate) → Ф-F4 (экспорт через
    reports). По одному ходу, движок не трогается. Дальше Ф-F1 → Ф-G.
    Ф-F1 ✅ (v0.60.734): scs-design/index.html +статичный read-only
    бейдж дисциплины-контекста (🔌 СКС/data) + .sd-disc-ctx CSS.
    0 JS, 0 изменений движка (§5.1). Дальше Ф-F1b (scs-config через
    JS-корень) → Ф-F2 (sidebar-аккордеон) → Ф-F3 → Ф-F4 → Ф-G.
    Ф-F1b ✅ (v0.60.735): scs-config renderProjectBadge() +идемпотентный
    бейдж #sc-disc-ctx соседом #sc-project-badge + .sc-disc-ctx CSS.
    0 изменений движка (компоновка/контент/матрица/владение данными
    не затронуты, §5.1). Дальше Ф-F2 (sidebar-аккордеон) → Ф-F3
    (header/project-context/role-gate) → Ф-F4 (экспорт reports) → Ф-G.
    Ф-F2 ✅ (v0.60.736): scs-design _initSidebarAccordion() — блоки
    .sd-plan-side-block → single-open (первый раскрыт, клик
    переключает), одноблочный sd-links-side не сворачивается;
    +CSS стрелка/collapse. memory:sidebar_accordion. UI-chrome,
    0 изменений движка (§5.1). Дальше Ф-F3 (header/project-context/
    role-gate дисциплины data) → Ф-F4 (экспорт reports) → Ф-G.
    Ф-F3 ✅ (v0.60.737): шапка/project-context уже унифицированы;
    добавлен role-gate дисциплины data как read-only индикатор
    (canCreateDiscipline, Ф-D) в бейдж обоих scs-* — «🔒 роль без
    прав на data» только для internal-роли без прав; не блокирует
    действия; не-internal → нет пометки (нулевая регрессия). Дальше
    Ф-F4 (экспорт плана зала через shared/report/* blocks) → Ф-G.
    Ф-F4 ✅ (v0.60.738): scs-design +кнопка «📄 Отчёт (через Отчёты)»
    — buildScsReportBlocks() blocks[] (стойки+кабельный журнал из
    getVisibleLinks/getProjectInstances) → composeReport (выбор
    шаблона, PDF/DOCX модулем reports/), не локальный HTML
    (memory:reports_via_module). SVG/PNG/CSV сохранены. 0 изменений
    движка/данных (§5.1). **Ф-F ЗАВЕРШЕНА** (F1/1b бейдж · F2
    аккордеон · F3 role-gate · F4 отчёты).
    Ф-G ✅ (v0.60.739): shared/object-registry.js +aggregateRegistry
    (total/byKind/byDiscipline/byPort/demandKwTotal, чистая); tech-
    workspace «📋 Реестр объектов» +сводная строка «Σ Сводно…» —
    технолог-ГИП единая точка сборки BOM по всем дисциплинам через
    реестр (§6, memory D7). Read-only, fail-soft.
    **План X.4.5.3 Вариант I ЗАВЕРШЁН ПОЛНОСТЬЮ**: Ф-A контракт ·
    Ф-B page.discipline · Ф-C контекст-палитра · Ф-D дисципл-RBAC ·
    Ф-E реестр объектов+порты · Ф-F СКС в оболочку · Ф-G агрегация.
    Задел §10 (технолог-ГИП lifecycle + SPATIAL-BASE) — отдельные
    будущие фазы (движок пространства/lifecycle-машина, после Ф-G).
