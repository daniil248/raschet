...
> **Статус:** v0.60.521 (2026-05-16). Текущий приоритет — **архитектурная
...
- [~] X.1.3 Упрочнение границ + гашение allowlist (Фаза 2).
  Сделано: #1 psychrometrics-core → `shared/calc/` (shim, meteo→
  psychrometrics устранён). Осталось: cooling→meteo×2, service→
  meteo, service→cooling/calc, js/main→reports-seed, battery-seed→
  battery; затем `projects/project.js` (15+ чужих LS) и `*-bridge.js`
