...
> **Статус:** v0.60.520 (2026-05-16). Текущий приоритет — **архитектурная
> модуляризация** (Фаза 0/1 + Cross-cutting X.1): разделить на Ядро +
> независимые модули по контрактам, zero-build, поэтапно. (Чистка
> структуры — удалён мёртвый код/мусор — выполнена, см. архив.)
...
- [ ] **1.2b** `tools/gen-modules-json.mjs` + CI блокирующий. (Фаза 1)
- [ ] **1.3** Per-module `manifest.json` (схема §B плана) → генерация
  корневого `modules.json` v1.2.0. `arch:Фаза 7.2,43-Etap2`
...
- [ ] X.1.3 Упрочнение мостов: `projects/project.js` (15+ чужих LS) и
  все `*-bridge.js` → через helpers (Фаза 2). Гасить allowlist.
