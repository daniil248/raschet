...
> **Статус:** v0.60.579 (2026-05-17). Текущий приоритет — **архитектурная
...
- [~] X.4.2 Cross-discipline schemes (узел в неск. дисциплинах). `arch:47.4.3`
  ...
  (user-params-sacred). Осталось: UI назначения членства + роутинг
  расчёта по дисциплине — отдельной одобренной под-фазой (CORE-риск).
...
- [~] X.4.4 Multi-discipline сводный отчёт + coordinated revisions. `arch:2.X.2,2.X.4`
  ...
  результаты передаёт потребитель). Осталось: producer (UI прогоняет
  методики → sections) + coordinated revisions — под-фазой.
