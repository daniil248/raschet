...
> **Статус:** v0.60.546 (2026-05-16). Текущий приоритет — **архитектурная
...
- [ ] X.5.0 **Importmap-развязка путей** (enabler физ. реструктуризации
  ...
  Поведение идентично; будущий move = правка ОДНОЙ карты.
  **Физ. переезд — ПИЛОТ ✅ (v0.60.546): `cooling/` → `apps/cooling/`**
  (git mv; depth-fix HTML; importmap-ключ `cooling/` в 31 HTML на
  `../apps/cooling/`; MODULE_PATHS; modules.json; hub/modules
  extractId стал move-proof по last-segment; разрозненные nav-ссылки
  + catalog-bridge). Bare-импорты cooling в др. модулях не менялись
  (авто-резолв через importmap — выигрыш развязки). Остаток: массовый
  переезд по этому рецепту в `apps/`+`lib/`+`platform/` (по группам).
