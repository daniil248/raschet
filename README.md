# Genesis Engineering Tools (GE Tools)

Набор инженерных и управленческих модулей и инструментов для широкого спектра проектных задач (электроснабжение, СКС, холодоснабжение, ГОТВ, метео, логистика, сервис/ТО, технолог-ГИП, отчёты и др.). Каждый модуль — самостоятельное приложение на общей платформе; общаются через контракты, единый проектный неймспейс и каталоги.

**Конструктор схем** — один из модулей (принципиальные схемы электроснабжения: блок-схема «источники → щиты → ИБП → потребители», автоматический расчёт нагрузки по активным цепям с учётом АВР по приоритетам, параллельной работы входов, резервных генераторов и КПД ИБП). Поддерживает режимы (аварии), вход через Gmail и совместное редактирование проектов.

## Текущее состояние приложения: 100% ФУНКЦИОНАЛЬНО ✅

Приложение можно использовать полностью. Любые ошибки `permission-denied`,
которые могут появляться в консоли DevTools, относятся **только к функциям
совместного редактирования в реальном времени** (presence, locks, sync чужих
курсоров) и не блокируют локальную работу — расчёт, схема, сохранение
проектов в `localStorage` работают независимо.

Чтобы убрать эти ошибки и включить полноценное real-time-сотрудничество,
**нужно один раз применить правила безопасности Firestore** из файла
...
firebase login
firebase deploy --only firestore:rules --project raschet-f1b2e
```

**Вариант 2 — через Firebase Console** (без CLI):

1. Открыть [console.firebase.google.com/project/raschet-f1b2e/firestore/rules](https://console.firebase.google.com/project/raschet-f1b2e/firestore/rules)
2. Скопировать содержимое файла `firestore.rules` → вставить в редактор → **Publish**

После деплоя обновить страницу приложения (F5) — ошибки `Missing or
insufficient permissions` пропадут, presence/locks/multiplayer заработают.

Подробности по правилам — внутри файла `firestore.rules` (комментарии).
...
Raschet — это **мульти-модульная инженерная платформа** (Конструктор
схем + ~28 подключаемых модулей: Технолог, ИБП, холод, СКС, АКБ, ДГУ,
трансформатор и др.), а не одно приложение.

```
raschet/
├── index.html / login.html / hub.html  — точки входа сайта (платформа)
├── app.js / app.css                     — редактор схем (window.Raschet)
├── modules.json                         — реестр модулей
├── js/         — CORE: engine/ (recalc/render/serialization), calc/, methods/
├── shared/     — общие контракты, мосты, каталоги (shared/contracts/, catalogs/)
├── <модуль>/   — папка на каждый модуль (cooling/, ups-config/, …) — см. ниже
├── tools/ scripts/ — CI-утилиты (boundary-lint, gen-modules-json, audit)
└── .github/workflows/ — CI (GitHub Actions)
```

> **Почему все папки модулей в корне, а не в `modules/`?** Это
> требование zero-build + GitHub Pages (сайт раздаётся из корня
> ветки, импорты — сырые относительные). Плоский корень здесь —
> *правильная* форма, а не беспорядок. Полная легенда раскладки,
> закон импортов и канонический скелет модуля (`calc/` + `ui/` +
> `manifest.json` + `README.md`) — в **[`ARCHITECTURE.md`](./ARCHITECTURE.md)**.
> В каждой папке модуля есть свой `README.md` (за 15 строк — что это
> и куда добавлять новое).
