...
function renderRight() {
  ...
  // Показываем норму + активные коэффициенты, чтобы пользователь видел,
  // что все узлы расчёта используют одну методику.
  const kA = safetyFactor(inst.norm, 'A');
  const kB = safetyFactor(inst.norm, 'B');
  const k1 = leakageK1(inst.norm);
  $('i-norm').textContent = normLabel;
  $('i-norm').title = `Коэффициенты методики (применяются ко всем узлам расчёта):\n`
    + `• k безопасности Cн = k·Cmin: класс A → ${kA}, класс B → ${kB}\n`
    + `• k1 (утечки в дежурном режиме): ${k1}\n`
    + `• плотности/скорости авто-DN и авто-коллектора: halocarbon ρ=1200/v=12, inert ρ=120/v=40, CO₂ ρ=700/v=15\n`
    + `• основная формула mp — СП 485 Прил. Д (совместима со всеми нормами при подставленных k и k1).`;
  ...
  try {
    ...
    const zoneResults = dir.zones.map(z => {
      ...
      const r = Annex.compute({
        ...
        // k1 — коэффициент утечек в дежурном режиме по выбранной норме
        // (единая методика: СП 485 1.05; СП РК 1.05; ISO 14520 1.00 — входит
        // в design concentration; NFPA 2001 1.00 — аналогично).
        k1: leakageK1(inst.norm),
      });
...
/** Коэффициент утечек k1 при расчёте массы ГОТВ (mg = k1·(mp+mtr+n·m1)) —
 *  по выбранной методике. СП 485 / СП РК закладывают 5%-й запас (k1 = 1.05);
...
function renderIso(dirId) {
  ...
  if (V3.showSup) {
    supports.forEach(sup => {
      ...
      const pp = P(p3);
      if (sup.type === 'rigid') {
        // Жёсткая (неподвижная) — оранжевый заштрихованный квадрат с крестом внутри
        svg += `<g pointer-events="none"><title>Жёсткая (неподвижная) опора</title>`
             + `<rect x="${pp.x-5}" y="${pp.y-5}" width="10" height="10" fill="#f9a825" stroke="#6d4c00" stroke-width="1.4"/>`
             + `<line x1="${pp.x-5}" y1="${pp.y-5}" x2="${pp.x+5}" y2="${pp.y+5}" stroke="#6d4c00" stroke-width="1"/>`
             + `<line x1="${pp.x-5}" y1="${pp.y+5}" x2="${pp.x+5}" y2="${pp.y-5}" stroke="#6d4c00" stroke-width="1"/>`
             + `</g>`;
      } else {
        // Скользящая (подвижная) — коричневый треугольник «∇» со «скользящей» чертой внизу
        // (типовое обозначение свободной опоры на изометрии трубопровода)
        svg += `<g pointer-events="none"><title>Скользящая (подвижная) опора</title>`
             + `<polygon points="${pp.x-6},${pp.y-5} ${pp.x+6},${pp.y-5} ${pp.x},${pp.y+4}" fill="#ffe0b2" stroke="#8d6e63" stroke-width="1.4"/>`
             + `<line x1="${pp.x-7}" y1="${pp.y+6}" x2="${pp.x+7}" y2="${pp.y+6}" stroke="#8d6e63" stroke-width="1.4"/>`
             + `</g>`;
      }
    });
  }
...
function findAxisConflict(pipeline, nodeId, axis) {
  ...
  if (incoming) {
    ...
    if (isTee && !isVert(incoming.axis) && axis === '-y' && isLiquidPhase) {
      // Запрещаем опускание только для T-фитинга на горизонтали: на «развилке»
      // вся жидкая фаза преимущественно уйдёт в опущенный отвод, вапор — в
      // боковой (асимметричный сток). Для простого колена (без ответвлений)
      // −Y допустимо — это штатный спуск к насадку.
      return `T-отвод вниз (−Y) на горизонтальной магистрали недопустим для ${agentType === 'co2' ? 'CO₂' : 'halocarbon'}: на развилке жидкая фаза стекает в опущенную ветвь, в боковую уходит пар (СП 485 / NFPA 2001 / FSSA). Используйте простой поворот (узел без ответвлений) или опускайте трубу от вертикального стояка.`;
    }
    ...
    const anyVert  = children.some(s =>  isVert(s.axis));
    if (isVert(axis) && anyHoriz) {
      return `От коллектора уже идут горизонтальные участки — вертикальный отвод в этом же узле не допускается (фитинги одной плоскости).`;
    }
    if (!isVert(axis) && anyVert) {
      return `От коллектора уже идёт вертикальный участок — горизонтальный отвод в этом же узле не допускается (фитинги одной плоскости).`;
    }
  }
  return null;
}
  ...
  mountHelp({
    ...
    usage: `
      ...
      <ul>
        ...
        <li>Cн нормируется автоматически по выбранному ГОТВ и классу пожара (Cн = k·Cmin, k = 1.2 для A, 1.3 для B — СП 485 п. 9.1.1), поле защищено от правки.</li>
        <li>Pпр выбирается из выпадающего списка типовых конструкций (витражи 0.001 … монолитные наружные ж/б 0.020 МПа).</li>
      </ul>
      <h4>Правила построения трубопровода (применяются автоматически, нарушения блокируются)</h4>
      <ol>
        <li><b>Насадок = лист дерева.</b> В узле с установленным насадком нельзя строить дальнейшие участки, и наоборот — нельзя ставить насадок в узле, где уже есть отводы.</li>
        <li><b>Одна плоскость на узел — по направлению потока.</b> Правило применяется к оси ВХОДЯЩЕГО участка (от баллона к насадкам): если вход горизонтальный, вертикальные отводы запрещены (жидкая фаза halocarbon расслаивается); если вход вертикальный — отводы в любую сторону допустимы. От коллектора (root) — держится одна плоскость среди детей.</li>
        <li><b>Максимум 3 трубы в узле</b> — T-фитинг. Крестовые соединения (4 трубы) запрещены.</li>
        <li><b>Однократное направление.</b> Кнопки X±/Y±/Z± блокируются (серые, зачёркнутые) для занятых направлений от выбранного узла — дважды построить «в ту же сторону» нельзя, как и «назад в родителя».</li>
        <li><b>Шаг длины 0,05 м.</b> Ввод длины и drag-resize узла привязаны к сетке 0,05 м (снап через Math.round(L/0.05)·0.05).</li>
        <li><b>Минимальная прямая L ≥ 10·DN</b> между фитингами (СП 485). Нарушения выводятся в «Предупреждениях».</li>
        <li><b>T-отвод вниз запрещён для halocarbon.</b> Т-отвод в −Y <em>на развилке</em> от горизонтальной трубы нарушает симметрию расхода (жидкость в отвод, вапор в бок). Простой поворот вниз (узел без других ответвлений) — разрешён.</li>
      </ol>
      <h4>Авто-подбор диаметра (DN) трубопровода</h4>
      <ul>
      ...
      <h4>Размеры насадков</h4>
      <ul>
        <li><b>R-360</b> (потолочный, 360°) — присоединение = DN трубы (равное проходное сечение).</li>
        <li><b>R-180</b> (боковой, 180°) — на 1 шаг меньше DN трубы (дросселирует поток).</li>
        <li><b>Радиальный</b> — равен DN трубы.</li>
        <li>Подпись DN выводится на 3D-схеме и в спецификации.</li>
      </ul>
      <h4>Правила расстановки опор трубопровода</h4>
      <ol>
        <li><b>Жёсткая (неподвижная) опора</b> — оранжевый квадрат с крестом — ставится у каждого фитинга/насадка, на <code>pad = min(0.3 м; 0.1·L)</code> от узла со стороны подходящего участка. Фиксирует направление трубопровода и воспринимает осевые усилия.</li>
        <li><b>Скользящая (подвижная) опора</b> — бежевый треугольник ∇ на горизонтальной «подошве» — ставится вдоль прямых участков между жёсткими. Расстояние между любыми двумя соседними опорами не превышает <code>supportStep(DN)</code>.</li>
        <li>Алгоритм: пролёт <code>span</code> между жёсткими делится на <code>⌈span/step⌉</code> равных частей, в промежутках равномерно ставятся скользящие.</li>
        <li><b>Максимальный шаг <code>supportStep(DN)</code></b> (СП 485, общестроительные нормы): DN15 — 2.0 м · DN20 — 2.5 м · DN25 — 2.5 м · DN32 — 3.0 м · DN40 — 3.5 м · DN50 — 4.0 м · DN65 — 4.5 м · DN80 — 5.0 м · DN100 — 6.0 м.</li>
        <li>Опоры сводятся в раздел «Опоры трубопровода» спецификации (кол-во жёстких / скользящих, CSV).</li>
      </ol>
      <h4>Авто-подбор типоразмера баллона</h4>
      <ul>
        <li>Пользователь не выбирает ёмкость баллона — алгоритм перебирает варианты серии и выбирает наименьший <code>ob</code> (литров), при котором число баллонов на батарею в самом загруженном направлении <code>max(n) ≤ 4</code>. Если такого нет — берётся самый большой.</li>
        <li>Селектор в диалоге установки теперь только отображает подобранный вариант.</li>
      </ul>
      <h4>Единая методика по всем узлам расчёта</h4>
      <p>Норма, выбранная в диалоге установки (СП 485 / СП РК / NFPA 2001 / ISO 14520), применяется ко всем узлам расчёта согласованно:</p>
      <ul>
        <li><b>Cн = k · Cmin</b> — k по норме: СП 485/СП РК — 1.2 (A), 1.3 (B); ISO 14520 — 1.2 (A), 1.3 (B/C); NFPA 2001 — 1.2 (A), 1.35 (B), 1.2 (C).</li>
        <li><b>k1</b> (утечки в дежурном, множитель в mg = k1·(mp+mtr+n·m1)): СП 485 / СП РК — 1.05; ISO 14520 / NFPA 2001 — 1.00 (запас уже включён в design concentration).</li>
        <li><b>Авто-DN и авто-коллектор</b> работают по одинаковым ρ/v для всех норм (физика потока не меняется).</li>
        <li><b>Основная формула mp</b> — СП 485 Прил. Д (совместима с ISO 14520 §7 / NFPA 2001 §5 при подстановке k и k1; различия в ссылках и оформлении отчёта).</li>
        <li>Активные коэффициенты видны в правой сводке (наведите на поле «Норматив»).</li>
      </ul>
      <h4>Интеграция с проектом</h4>
      <ul>
        <li>Если модуль открыт из главного Конструктора (или другого модуля, где задан проект), поля «Объект / Заказчик / Договор» заполняются из <code>localStorage['raschet.activeProject.v1']</code>.</li>
        <li>«Общая аксонометрия»: централизованная система / 1 сборка — объединённый просмотр; модульная с несколькими сборками — запрос выбора направления.</li>
      </ul>
    `,
    calcs: `
      <h4>Формулы</h4>
      ...
      <ul>
        ...
        <li>Re = v·D/ν, ν (halocarbon) ≈ 1.8·10⁻⁷ м²/с, шероховатость Δ = 0.05 мм (сталь).</li>
      </ul>
      ...
      <p><b>Опоры трубопровода:</b></p>
      <ul>
        <li>Жёсткие — у фитингов/насадков, на расстоянии pad = min(0.3 м; 0.1·L) от узла.</li>
        <li>Скользящие — между жёсткими, количество n = ⌈span/step⌉ − 1, равномерно.</li>
        <li>step(DN) — табличная функция: DN15→2.0 · DN20→2.5 · DN25→2.5 · DN32→3.0 · DN40→3.5 · DN50→4.0 · DN65→4.5 · DN80→5.0 · DN100→6.0 м.</li>
      </ul>
      <p><b>Размер насадка:</b></p>
      <ul>
        <li>nozzleSize(DN, R-360)  = DN трубы;</li>
        <li>nozzleSize(DN, R-180)  = DN<sub>−1</sub> (на шаг меньше по ряду);</li>
        <li>nozzleSize(DN, radial) = DN трубы.</li>
      </ul>
      <p><b>Автоматическая концентрация Cн:</b></p>
      <ul>
        <li>Cн = k · Cmin, где Cmin — минимальная огнетушащая концентрация из каталога AGENTS (Cmin_A для класса A, Cmin_B для B, об. %), k = 1.2 (A) / 1.3 (B) — СП 485 п. 9.1.1 / ISO 14520 §7.5.</li>
      </ul>
    `,
  });
  mountFooter({
