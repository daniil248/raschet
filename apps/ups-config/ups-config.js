...
// v0.60.565 (Фаза 3, memory:reports_via_module): отчёт о конфигурации
// ИБП формируется через модуль reports/ (blocks[] API) вместо прямого
// HTML+window.open. Подпрограмма отдаёт структуру; стили/поля — reports/.
async function _printUpsReport() {
  const comp = wizState.composition;
  if (!comp) { flash('Сначала выберите конфигурацию (Шаг 4)', 'warn'); return; }
  const u = comp.ups || {};
  const fi = comp.fitInfo || {};
  ...
  let Report;
  try { Report = await import('shared/report/index.js'); }
  catch (e) { flash('Не удалось загрузить модуль отчётов: ' + (e.message || e), 'error'); return; }
  const B = Report.blocks;
  const rw = (k, v, unit) => [String(k), String(v ?? '—'), String(unit || '')];
  const paramCols = ['Параметр', { text: 'Значение', align: 'right' }, { text: 'Ед.', align: 'center' }];

  const content = [
    B.h1('Отчёт о подобранной конфигурации ИБП'),
    B.caption(`Модель: ${upsLabel} · Дата: ${new Date().toLocaleString('ru-RU')}`),
    B.h2('1. Исходные требования'),
    B.table(paramCols, [
      rw('Нагрузка', rq.loadKw, 'кВт'),
      rw('Автономия', rq.autonomyMin, 'мин'),
      rw('Резервирование', rq.redundancy, ''),
      rw('Тип ИБП', rq.upsType ? ((getUpsType(rq.upsType) || {}).label || rq.upsType) : 'любой', ''),
      rw('V_DC (по паспорту ИБП)', `${u.vdcMin || '—'}…${u.vdcMax || '—'}`, 'В'),
      rw('cos φ', rq.cosPhi, '—'),
      rw('Фазы', rq.phases, ''),
      rw('АКБ', wizState.batteryChoice === 'skip' ? 'пропущены' : (battery ? [(battery.supplier || ''), (battery.model || battery.id || '')].filter(Boolean).join(' ') : '—'), ''),
    ]),
  ];
  ...
  content.push(
    B.h2('2. Подобранная конфигурация'),
    B.table(paramCols, [
      rw('Модель', upsLabel, ''),
      rw('Тип', upsTypeName, ''),
      isModular ? rw('Корпус (frame)', u.frameKw, 'кВт') : null,
      isModular ? rw('Модуль', u.moduleKwRated, 'кВт') : null,
      isModular ? rw('Установлено модулей', `${fi.installed} из ${u.moduleSlots || '—'}`, 'шт.') : null,
      isModular ? rw('Рабочих модулей', fi.working, 'шт.') : null,
      isModular ? rw('Резерв', fi.redundant, 'шт.') : null,
      rw('Реальная мощность', fi.realCapacity || fi.usable, 'кВт'),
      rw('Итоговая мощность', fi.usable, 'кВт'),
      rw('КПД DC–AC', u.efficiency, '%'),
      rw('cos φ ИБП', u.cosPhi, '—'),
      rw('Топология (IEC 62040-3)', u.topology, ''),
      rw('V_DC мин/макс', `${u.vdcMin || '—'}…${u.vdcMax || '—'}`, 'В'),
      rw('Входов / Выходов', `${u.inputs || 1} / ${u.outputs || 1}`, ''),
    ].filter(Boolean)),
    B.h2('3. Стоимость (оборудование ИБП)'),
    B.table(paramCols, [
      rw('Цена за единицу', priceUnit, ''),
      rw('Количество', fi.installed, 'шт.'),
      rw('Итого ИБП', priceTotal, ''),
    ]),
    B.caption('АКБ подбирается отдельно в «Калькуляторе АКБ». Цены пополняются в модуле «Каталог и цены».'),
  );

  if (Array.isArray(comp.composition) && comp.composition.length) {
    content.push(B.h2('4. Состав комплекта (BOM)'));
    content.push(B.table(
      ['Поз.', 'Наименование', { text: 'Кол-во', align: 'right' }, { text: 'Цена за ед.', align: 'right' }, { text: 'Сумма', align: 'right' }],
      comp.composition.map((it, idx) => {
        ...
        return [String(idx + 1), name, String(qty), unit, sum];
      }),
    ));
  }
  ...
  if (battery && wizState.batteryChoice !== 'skip') {
    ...
    content.push(B.pageBreak(), B.h2(`${sectionNo}. Аккумуляторная батарея`));
    content.push(B.table(paramCols, [
      rw('Производитель', battery.supplier, ''),
      rw('Модель', articleUpper, ''),
      rw('Тип АКБ', battery.chemistry === 'li-ion' ? 'Li-Ion (LFP)' : 'VRLA', ''),
      rw('Напряжение блока', battery.blockVoltage, 'В'),
      rw('Ёмкость блока', battery.capacityAh, 'А·ч'),
      battery.dcVoltage != null ? rw('Напряжение DC-шины', battery.dcVoltage, 'В') : null,
      battery.blocksPerString != null ? rw('Блоков в цепочке', battery.blocksPerString, 'шт.') : null,
      battery.strings != null ? rw('Параллельных цепочек', battery.strings, 'шт.') : null,
      battery.totalBlocks != null ? rw('Всего блоков (модулей)', battery.totalBlocks, 'шт.') : null,
      battery.totalKwh != null ? rw('Суммарная ёмкость АКБ', Number(battery.totalKwh).toFixed(1), 'кВт·ч') : null,
      battery.autonomyMin != null ? rw('Расчётная автономия', Number(battery.autonomyMin).toFixed(1), 'мин') : null,
      battery.endV != null ? rw('Конечное напряжение элемента', battery.endV, 'В') : null,
      battery.invEff != null ? rw('КПД инвертора (учёт)', (Number(battery.invEff) * 100).toFixed(0), '%') : null,
      battery.mode ? rw('Режим расчёта', battery.mode === 'autonomy' ? 'Прямой (автономия по блокам)' : 'Обратный (блоки по автономии)', '') : null,
    ].filter(Boolean)));
    if (battery.derate && (battery.derate.kAge || battery.derate.kTemp || battery.derate.kDesign)) {
      const d = battery.derate;
      content.push(B.h3('Коэффициенты расчёта (IEEE 485 / IEC 62040)'));
      content.push(B.table(paramCols, [
        d.kAge != null ? rw('k_age (старение, EOL)', Number(d.kAge).toFixed(2), '—') : null,
        d.kTemp != null ? rw('k_temp (температура)', Number(d.kTemp).toFixed(2), '—') : null,
        d.kDesign != null ? rw('k_design (запас)', Number(d.kDesign).toFixed(2), '—') : null,
        d.kTotal != null ? rw('k_total', Number(d.kTotal).toFixed(3), '—') : null,
        d.vdcSafetyPct != null ? rw('Окно V_DC', '±' + Number(d.vdcSafetyPct).toFixed(1), '%') : null,
        d.socMinPct != null ? rw('Резерв SoC (Li-ion)', Number(d.socMinPct).toFixed(0), '%') : null,
      ].filter(Boolean)));
    }
    content.push(B.caption('Расчёт выполнен в подпрограмме «Расчёт АКБ»; полный отчёт с графиком разряда доступен там же.'));
    const battComp = _buildBatteryComposition();
    if (battComp && battComp.length) {
      const totalBlocksAll = (battComp.find(it => it.role === 'battery-block') || {}).qty || 0;
      const battTopologyR = wizState.batteryTopology || (_getUpsUnits(wizState.composition).unitCount > 1 ? 'per-unit' : 'shared');
      const battSetsQtyR = (battTopologyR === 'shared') ? 1 : _getUpsUnits(wizState.composition).unitCount;
      content.push(B.h3('Состав комплекта АКБ (BOM)'));
      content.push(B.caption(`Топология: ${battTopologyR === 'per-unit' ? 'на каждый ИБП (per-unit)' : 'общая шина (shared)'} · Комплектов: ${battSetsQtyR}${totalBlocksAll > 0 ? ` · ${totalBlocksAll} блок(ов) всего` : ''}`));
      content.push(B.table(
        ['Поз.', 'Наименование', { text: 'Кол-во', align: 'right' }, { text: 'На комплект', align: 'right' }],
        battComp.map((it, idx) => [
          String(idx + 1),
          (it.label || it.role || '—') + (it.elementId ? ` (${it.elementId})` : ''),
          String(it.qty),
          String(it.perSet ? it.perSet : (it.role === 'battery-set' ? '—' : (it.qty / battSetsQtyR).toFixed(0))),
        ]),
      ));
    }
  }
