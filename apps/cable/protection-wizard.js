...
// Время отключения tk (с) — фактическое время срабатывания, зависит от
// характеристики автомата И расстояния в цепочке.
//
// Для MCB B/C/D с мгновенным электромагнитным расцепителем при I_k,
// значительно превышающем I_mag (что типично на вводном и среднем уровне),
// время отключения — 10–50 мс. НЕ путать с t_k = 0.4 с — это время для
// селективной задержки MCCB/ACB с расцепителем с выдержкой.
//
// Ключ: [loadType][distance] → tk (с)
const TK_MATRIX = {
  lighting:   { near: 0.10, medium: 0.05, far: 0.02, veryfar: 0.02 },  // MCB B, 5×In мгн.
  mixed:      { near: 0.15, medium: 0.08, far: 0.03, veryfar: 0.02 },  // MCB C, 10×In
  motor:      { near: 0.20, medium: 0.10, far: 0.05, veryfar: 0.03 },  // MCB D, 20×In (пусковые)
  industrial: { near: 0.40, medium: 0.25, far: 0.15, veryfar: 0.10 },  // MCCB с задержкой/селективн.
};
...
function calcFromAnswers() {
  ...
  // tk зависит от типа автомата (load) И уровня в цепочке (distance).
  // Для быстрых MCB B/C/D с мгновенным электромагнитным расцепителем на
  // удалённом уровне tk = 0.02–0.05 с (а не 0.15 как было раньше).
  // Это критично для модуля «Термическая стойкость к Ik»: S_min = I×√tk/k.
  const tkMatrix = TK_MATRIX[a.load] || TK_MATRIX.mixed;
  const tk = tkMatrix[a.distance] || 0.1;
...
function renderSummary() {
  ...
  content.innerHTML = `
    ...
    <div class="pw-hint" style="margin-top:14px;font-size:11px;line-height:1.55">
      <b>Про Ik:</b> точное значение запрашивается у энергосбыта (паспорт ТП). На скриншоте пользователя Ik=6 кА + tk=0.15 с привело к S_min=20.2 мм² (→ 25 мм²) — это характерно для вводного щита, а не для удалённого.<br>
      <b>Про tk:</b> это время срабатывания защиты при КЗ. Для автоматов B/C/D с мгновенным электромагнитным расцепителем при Ik &gt;&gt; In(магн.) реальное отключение — 10–50 мс, а не 150 мс. Используйте меньшее tk для удалённых/групповых линий — это радикально уменьшит требуемое сечение по термической стойкости.<br>
      <b>Совет:</b> если кабель «раздувается» с 1.5 мм² до 16–25 мм² — проверьте tk и Ik: скорее всего они завышены.
    </div>
  `;
}
