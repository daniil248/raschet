...
const MAGNETIC_BOUNDS = {
  ...
  K: { min: 8, max: 14 },
  Z: { min: 2, max: 3 },
  // MCCB/ACB — settable, здесь приведены типовые значения для
  // нерегулируемой или фабричной настройки short-time. Реальный
  // токоограничивающий участок может быть уже по таблицам производителя.
  MCCB: { min: 5, max: 10 },
  ACB:  { min: 5, max: 10 },
  // MV-выключатели (VCB/SF6, IEC 62271-100) — срабатывают по внешнему
  // реле (IEC 60255 / IEC 61850). Типовой definite-time overcurrent с
  // уставками Is=6…12×In, ts=0.05…0.3 с. Приведена упрощённая полоса.
  VCB: { min: 6, max: 12 },
  SF6: { min: 6, max: 12 },
};
