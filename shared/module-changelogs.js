...
export const CHANGELOGS = {
  ...
  'scs-config': [
    { version: '0.59.245', date: '2026-04-22', items: [
      '🔁 Двустороннее размещение оборудования (front/rear rails). У каждого устройства появилось поле mountSide: "front" | "rear" (default "front"). В таблице размещения добавлена колонка «Сторона» с селектором ▲front / ▼rear. На карте юнитов — новый переключатель «▲ фронт | ▼ тыл | ⇄ обе»: фронт и тыл показываются независимо (одни и те же U на разных сторонах — не коллизия), а в режиме «обе» рисуется вид сверху — ширина устройства пропорциональна его глубине.',
      '📐 Учёт глубины. В каталоге типов добавлен столбец «Гл. мм» (depthMm). Дефолты по kind: сервер 750, коммутатор 400, патч-панель 200, кабельный органайзер 60, ИБП 700, консоль/KVM/монитор 600. В стойке используется rack.depth (из rack-config) с дефолтом 1000 мм.',
      '🚫 Коллизии по глубине. При установке устройства (front или rear) на U, где на противоположной стороне уже стоит устройство, проверяется dF + dR + запас 80 мм ≤ rack.depth. При нарушении — toast «Коллизия по глубине на U{n}: front X мм + rear Y мм + запас 80 мм = Z мм > стойка W мм», установка отменяется. В неглубоких стойках (600–800 мм) устройства глубиной >=rack.depth вовсе не принимаются.',
      '🎯 Выбор стороны при drag-n-drop. При перетаскивании из каталога/тележки/склада: в режиме «обе» — по половине карты, на которую бросаете (левая=front, правая=rear). В режимах «фронт»/«тыл» — на текущую сторону. Модификатор Alt переключает сторону в любой момент.',
      '🔎 Визуализация глубины в режиме «⇄ обе»: градиент (светло→темно — от рельса внутрь), штриховка (front — наклон 45°, rear — противоположный) и полупрозрачный цвет kind сверху. Под каждым устройством подпись «{depth} мм · ▲фронт/▼тыл».',
      'Миграция: старые каталоги получают depthMm по kind при загрузке; старые contents получают mountSide="front". Шаблоны сборок (assemblyTemplates) теперь сохраняют mountSide и offsetMmFromRail.',
      'Файлы: scs-config/scs-config.js (state.sideView, checkDepthFit, detectConflicts per-side + depth, renderUnitMap с defs/railAxis/both-view, renderCatalog +depthMm, renderContents +side, pickDragSide), scs-config/scs-config.css (.sc-sideview/.sc-sv-btn, .sc-side-sel, .sc-devband-*), scs-config/rack.html (переключатель стороны + hint).',
    ] },
