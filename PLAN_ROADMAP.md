...
- **1.20.67 (v0.57.43)** — Инспектор ИБП: placeholder «авто ≈ N» в полях
  номиналов автоматов (по autoUpsBreakerNominals). Подсказка о том, что
  эти автоматы в составе ИБП и не попадают в спецификацию; значения
  применяются к линиям от/к ИБП (QF1/QF2/QF3).
...
- **1.20.65 (v0.57.41)** — Автоматы ИБП как встроенные в ИБП.
  - Линия ОТ ИБП: номинал берётся из `n.outputBreakerIn` (QF3); если
    ...
    инвертора; флаги `_breakerInternal=true`, `_breakerExcludeFromBom=true`.
  - Линия К ИБП (основной/байпасный вход): номинал из `inputBreakerIn` /
    `inputBypassBreakerIn` (QF1 / QF2); те же флаги. Направление
    определяется по `c.to.port === 'bypass'` или `c.upsInputKind`.
  - `manualBreakerIn` на такой линии отменяет «встроенность» (внешний
    автомат в щите после ИБП — уже часть спецификации).
  - Inspector/conn: показывает метку источника («QF3 — выход ИБП» и т.п.)
    и пояснение «в спецификацию не попадает, поставляется в составе ИБП».
  - Если флаг hasXXXBreaker=true, но номинал не указан — UI просит
    уточнить в параметрах ИБП; `_breakerIn=null` до уточнения.
- **1.20.64 (v0.57.40)** — MV BOM: каждая ячейка разбивается на отдельные
  позиции (корпус ячейки + основной аппарат + предохранители + реле
  защиты). VCB-ячейка → 3 строки (корпус + вакуумный выключатель +
  реле), fuse-switch ячейка → 3–4 строки (корпус + выключатель нагрузки
  + предохранители ×3 + реле). Depth-иерархия 1 → 2 для визуализации.
...
- **1.20.60 (v0.57.36)** — FIX: запас по автомату/кабелю для групповой
  ...
  (68.4 А) с per-line автоматом (25 А). Теперь при N приборов × N per-line
  автоматов margin = (InPerLine − Iper)/Iper, cable margin = (Iz − Iper)/
  Iper. Блок «Как получено» для групповой нагрузки теперь описывает
  per-line правило координации.
  Файл: js/engine/inspector/conn.js (строки ~461-476, 536-555).
- **1.20.59 (v0.57.35)** — Авто-номинал многосекционного щита.
  Для sectioned-контейнера поле «In, А» в инспекторе не показывается
  (уже было), но теперь номинал сборки определяется на recalc:
  - capacityA = max(секций) с повышением до ближайшего стандартного
    номинала BREAKER_SERIES, если максимальный протекающий _maxLoadA
    превышает сумму max-секций (сборка через СВ должна пропустить
    фактический ток).
  - В инспекторе sectioned-панели добавлен info-блок с расчётом:
    «Номинал сборки: N А (авто) = max(секций) X А, повышен до ≥ Imax».
  Файлы: js/engine/recalc.js (второй проход, строки ~2188-2210);
  js/engine/inspector/panel.js (isSectioned, строки ~343-360);
  js/engine/constants.js (import BREAKER_SERIES в recalc).
...
- **1.20.57 (v0.57.33)** — Многосекционный щит: одновременная нагрузка
  ...
  приводило к двойному учёту потребителей, достижимых через замкнутый СВ
  (bus-tie) из нескольких секций, и — что важнее — max-ток контейнера
  был суммой максимумов секций, а не реальной одновременной нагрузкой
  по всем потребителям.
  - Добавлен `_sectionedContainerAgg(container)` в recalc.js: BFS-обход
    downstream-дерева всех секций с общим visited-set (`seenNodes`,
    `seenConsumers`, `seenUps`) — каждый потребитель учитывается один
    раз, независимо от числа путей к нему.
  - P/Q считаются с учётом kUse × effectiveLoadFactor (режим), maxKw —
    по паспортной `demandKw × count` (100%), как и для обычных панелей.
  - ИБП: maxKw делится на средний КПД и добавляется зарядка; downstream
    через ИБП учитывается один раз (в upsMaxKw, не в основном maxKw).
  - Aux-нагрузка генераторов (auxDemandKw) учитывается с дедупом.
  - Второй проход в recalc теперь вызывает `_sectionedContainerAgg(n)`
    и пересчитывает токи (_loadA/_maxLoadA) при номинальном напряжении
    контейнера по агрегированному cosφ.
  Файл: js/engine/recalc.js (строки ~312-385, 2148-2192).
- **1.20.56 (v0.57.32)** — Standby group-scoped в N-1-отчёте.
  Проблема: подменный (isStandby) ранее считался универсальным
  резервом всех primaries, независимо от redundancyGroup. В реальности
  подменный ДГУ может заменять только конкретный генератор своего тира.
  Исправление в _computeSourceCapacity: standby со значением
  «Группа резерва» компенсирует отказ только в этой группе; standby без
  группы остаётся глобальным резервом. explain-строки теперь разделены
  по группам.
- **1.20.55 (v0.57.31)** — Агрегация параметров потребления для
  многосекционного щита. Было: на контейнере PNL1 в инспекторе всегда
  «Текущая: 0 kW · 0 A», «макс.ток: 0 А» — панель только суммировала
  _loadKw/_maxLoadKw секций, а все производные (_powerP/Q/S, _loadA,
  _maxLoadA, _cosPhi, _marginPct) не считались. Теперь:
  - recalc.js: для switchMode='sectioned' суммируем P, Q, _maxLoadA,
    _loadA секций (секции питают disjoint поддеревья — без дубля),
    cosφ = P/S агрегированный, токи пересчитываются при напряжении
    контейнера. Маржа номинала (_marginPct/_marginWarn) считается по
    суммарному max-току.
  - Добавлен второй проход после основного цикла, чтобы гарантировать
    что секции уже обсчитаны к моменту агрегации на контейнере (порядок
    итерации Map может быть произвольным).
  panelStatusBlock уже использует все эти поля — контейнер теперь
  показывает полный набор метрик так же, как обычный щит.
- **1.20.54 (v0.57.30)** — Взаимное резервирование источников (UX).
  Вопрос: как описать два трансформатора, питающих одну шину и взаимно
  резервирующих друг друга. Ответ: обоим присваивается одинаковая
  «Группа резерва» (например «T»), флаги backup/standby НЕ включаются.
  Модель уже реализована в _computeSourceCapacity (v0.57.20): для группы
  с ≥2 primaries availCap = sum − max (N-1 внутри группы).
  
  UX-улучшение: в инспекторе Источника под полем «Группа резерва»
  появился живой hint + сводка группы:
  - «🔗 В группе «T»: 2× параллельно, взаимный резерв N-1. Остаток при
    отказе одного: X кВт (из Y).»
  - Если участник всего один — подсказка «Задайте ту же группу другому
    источнику — они будут работать параллельно и резервировать друг
    друга.»
  Также исправлен hint под полем: «Два и более источников с одинаковой
  группой → параллельная работа + взаимный резерв.»
  
  Per-user UI preferences в serialization.js: ключи showHelp /
  autoCenterOnSelect теперь не сохраняются в проект (каждый участник
  collab-сессии управляет своими локально).
  Файлы: js/engine/inspector.js, js/engine/serialization.js.
...
- **1.20.50 (v0.57.26)** — FIX: иконки на карточке канала не соответствуют
  ...
  G → 'wall' (вместо одножильных с интервалами в воздухе), C и G
  рисовались одинаково. Также bundlingIconSVG для 'touching'
  рисовал окружности с зазором 4 единицы вместо касания.
  Исправлено:
  - A1 → новая иконка insulated_conduit (штриховка сверху+снизу + труба)
  - A2 → insulated_cable (штриховка + жилы без трубы)
  - G → air_spaced (3 жилы в ряд с маркером расстояния «≥Ø», без лотка)
  - C → оставлен 'wall', но жилы подняты ближе к штриховке стены
  - bundling 'touching' — окружности теперь действительно касаются
    (центры 18 и 30, r=6 → расстояние = 2r)
  Файл: js/engine/inspector/conn.js — channelIconSVG маппинг
  расширен (+3 case), bundlingIconSVG touching координаты.
...
- **1.20.49 (v0.57.24)** — filter-поиск в модалке «Проверки проекта».
  ...
  textContent каждой .pi-row (match = containsString). Пустые секции
  (H3 + следующий контейнер) скрываются, пока поле не пусто.
  Счётчик «N из total» справа от поля.
  Файл: js/main.js — input#pi-filter и handler после mount.innerHTML.
- **1.20.48 (v0.57.23)** — bulk-autofix амплитудной селективности.
  Проблема: в проектах с 40+ нарушений клик по каждой кнопке «✓ In_up»
  превращается в работу на десятки минут. Добавлена кнопка-заголовок
  «⚡ Исправить все (N)» рядом со счётчиком «Нарушения селективности».
  Собирает все fixable-пары (LV, с upstream.id), группирует по connId
  (несколько нарушений ниже одного upstream → берём max suggested),
  применяет одним snapshot'ом. Confirm-диалог + flash с результатом.
  Файл: js/main.js — selFixable[] собирается до рендера, кнопка
  #pi-sel-fix-all в заголовке, handler с group-by-conn.
- **1.20.47 (v0.57.22)** — MV BOM: ячейки РУ СН детализируются отдельно.
  Из очереди плана: «cells → детализация VCB/fuse как отдельные позиции».
  Каждая ячейка из n.mvCells (isMv-щит) выводится строкой в новой секции
  «Ячейки РУ СН» BOM с моделью «<Функция> · <breakerType> · <In>А»
  (напр. «Ввод · VCB · 1250А»). Агрегация по (type+breakerType+In),
  одинаковые ячейки из разных щитов складываются по qty; в notes —
  tag щита + functionCode.
  Файл: js/engine/bom.js — новый блок в main loop и секция в sectionOrder
  между «Щиты» и «ИБП».
...
- **1.20.44 (v0.57.19)** — FIX: разные сечения у симметричных линий
  ...
  жилу группового потребителя как отдельную цепь (`circuits = count`).
  Нарушение IEC 60364-5-52 §523.5 / Annex E: «параллельные проводники
  одной цепи считаются одной цепью для группового коэффициента».
  Из-за этого фидер с count=4 получал `grouping=4` (K_G≈0.65), а
  симметричный фидер с count=3 — `grouping=3` (K_G≈0.70), хотя
  per-line ток одинаковый. Результат: 4×4 мм² vs 3×6 мм².
  Исправлено: `circuits = 1` на каждую cпрсь в канале, независимо
  от count/parallel.

- **1.20.43 (v0.57.18)** — колонка «Резерв» в Таблице оборудования.
  Чекбокс inline (source/generator) — переключает `n.isStandby`
  без открытия инспектора, с snapshot() и rerender(). Сортируется.
  В CSV-экспорт добавлена колонка «Резерв» (Да/Нет / пусто для не-
  источников). Видимость управляется через меню столбцов, позиция
  между IP и «Связано».
  Файл: js/main.js — _EQUIPMENT_TABLE_COLUMNS, renderEquipmentTable
  (th/td + `.et-standby` handler), sortKey, exportEquipmentTableCsv.

- **1.20.42 (v0.57.17)** — N-1 проверка добавлена в «Проверки проекта»
  (`modal-project-issues`) и в CSV-экспорт. При недостатке резерва
  появляется секция «⚠ N-1 резервирование» с описанием: какой
  источник отказал (max nonStandby), сколько остаётся, сколько
  требуется, размер дефицита. Бейдж ошибок/предупреждений в
  сайдбаре учитывает N-1. Выделен общий helper `_computeN1(S)`,
  используемый и в Dashboard (v0.57.16), и в `_countProjectIssues`,
  и в `renderProjectIssues`, и в `exportProjectIssuesCsv`.
  Файл: js/main.js — новая функция `_computeN1`, секция в issues,
  строка в CSV-экспорт.

- **1.20.41 (v0.57.16)** — N-1 анализ резерва в Dashboard. Новая
  карточка «N-1 резерв» в ряду «Общий статус»: считает, хватит ли
  мощности оставшихся рабочих источников + одного самого мощного
  резервного (`isStandby`), если выходит из строя самый мощный
  рабочий источник. Формула: `availCap − max(nonStandbyCaps) +
  max(standbyCaps) ≥ totalLoad`. Зелёная ✓ если хватает, красная ✗
  если нет. Показывается только если источников ≥2 или есть резерв.
  Файл: js/main.js:renderDashboard — nonStandbyCaps/standbyCaps списки,
  capAfterFail, карточка в статус-ряду.
...
- **1.20.39 (v0.57.14)** — концепция резерва + fix расчёта нагрузки +
  ...
  **Нагрузка и мощность источников (исправление):**
  - Dashboard «Общая нагрузка» = Σ `demandKw × count` по всем потребителям
    (верхняя оценка: все включены одновременно). Раньше было Σ `_powerP`
    по активным — занижало на Ки и скрывало пиковую нагрузку.
  - Sidebar: «Источников» переименовано в «Общая мощность ист. питания».
    «Потребляется» (sum `_loadKw`) убрано как некорректное — при N+1
    резерве давало двойной учёт одних и тех же потребителей.
  - Добавлено «Доступно (с резервом)» — сумма capacity только
    НЕ-резервных источников.

  **Резервные источники:**
  - В инспекторе source/generator — чекбокс «Резервный (подменный)»
    (`n.isStandby`). Используется для подменного ДГУ, холодного резерва,
    второго ввода. Не учитывается в «доступной мощности».
  - Status bar: если есть резервные — процент загрузки считается от
    availCap, показывается «X / Y кВт (резерв N)».

  **Utility capacity (городская сеть):**
  - В модалке «Параметры источника» для subtype=utility добавлено поле
    «Разрешённая мощность по ТУ, кВт». Раньше `n.capacityKw` для utility
    нигде не задавалась → городская сеть не учитывалась в сумме
    источников. Теперь задаётся из ТУ сетевой организации вручную.

  **Roadmap + Changelog модули:**
  - `index.html`: `#modal-roadmap` + `#modal-changelog` с mini-md-парсером.
  - Кнопки в sidebar: «🗺 Roadmap» / «📜 Изменения».
  - Roadmap загружает `PLAN_ROADMAP.md` через fetch и рендерит в HTML
    (заголовки, списки, таблицы, код, bold, ссылки, подсветка версий).
  - Changelog парсит записи вида `- **1.20.X (vA.B.C)** — …`, сортирует
    по semver desc, добавляет live-поиск по версии/тексту. Ссылки на
    GitHub commits и PLAN_ROADMAP.md.

- **1.20.38 (v0.57.13)** — fix: TCC-кривая автомата не показывалась для
  групповых потребителей (count>1, не-serialMode). Для них recalc
  устанавливает `c._breakerIn = null`, а номинал хранит в
  `c._breakerPerLine` (по одному автомату на каждую параллельную линию,
  «3 × 6 A» в щите). `_buildConnTccPayload` проверял только `_breakerIn`
  → this-breaker item не добавлялся. Исправлено: fallback на
  `_breakerPerLine`, лейбл «ЭТА линия: MCB_C 3 × 6A (групповая)».
