...
function renderCoolCard(cu, isReadOnly, rooms) {
  ...
  return `<div class="tw-card" data-card-kind="cool" data-card-id="${cu.id}">
    ...
    <a class="tw-bind-btn tw-bind-btn-bound" style="text-decoration:none;display:inline-block;margin-left:6px"
       href="../apps/cooling/?project=${escAttr(_pid || '')}" target="_blank"
      ...
      <div class="tw-details-body">
        <div class="tw-card" data-card-kind="coolsys" data-card-id="-">
          ...
          <div class="tw-pue-actions">
            ...
            <a class="tw-pue-link" href="../apps/cooling/" target="_blank" title="Открыть cooling в новой вкладке без pre-fill (для просмотра существующих подборов).">↗ Открыть cooling</a>
          </div>
          ${(() => {
...
• Вручную — введите готовое значение из внешнего расчёта.">Режим:
          ...
          ${isCoolingMode ? `<div class="tw-pue-breakdown" title="Phase 22.4: PUE считается из реальной топологии активного подбора cooling (через simulateTopology). Если подбора нет — fallback на auto-режим.">
            ...
            <div class="tw-pue-actions">
              <a class="tw-pue-link" href="../apps/cooling/" target="_blank" title="Открыть модуль подбора в новой вкладке. Изменения после возврата применятся к PUE автоматически.">↗ Открыть «Подбор холодильных систем»</a>
            </div>
          </div>` : ''}
          ${isAuto ? (() => {
  ...
  root.addEventListener('click', async (e) => {
    ...
    if (openCoolingPrefill) {
      try {
        ...
        openEmbed(location.pathname + location.search, '../apps/cooling/', 'Технолог ЦОД');
      } catch (err) {
...
<p>В проекте создан связанный подбор холодильных систем «<b>${escHtml(coolSummary.selectionName)}</b>» (модуль <a href="../apps/cooling/?project=${escAttr(_pid)}" target="_blank">«Подбор холодильных систем»</a>) с ${coolSummary.optionCount} варианта${coolSummary.optionCount === 1 ? 'ом' : 'ми'} оборудования.</p>
<div class="summary">
...
const TW_MODULES = [
  ...
  { id: 'cooling',          icon: '❄', label: 'Подбор холодильных систем',
    ...
    href: '../apps/cooling/', hint: 'Подборы оборудования (чиллеры, DX, CRAC) с CAPEX/TCO/free-cooling.' },
