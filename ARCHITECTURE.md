...
> МОДУЛЬ, не ядро; «ядро программы» = ПЛАТФОРМА схемного редактора
> (`js/engine`: state/render/serialization/interaction) — универсальная
> база ВСЕХ типов схем; `js/calc` + `lib/*-methods` — calc-модули
> (`js/methods` УДАЛЁН: электрика → `lib/electrical-methods`, v.593).
> **D2** модули нативно импортируют calc-модули (реализовано в
> boundary-lint, v.599). **D3** Конструктор — универсальная база ВСЕХ
> типов схем с контекстом систем (ОТМЕНЯЕТ v0.60.277/278 «чистая
> электрика») — ⏳ ОТКРЫТО (крупный UX-редизайн). **D4/D5** метод-по-
> файлу + picker + мультинорма (РФ/КЗ/ISO/IEC) + region/version/
> enabled — ✅ по всем calc-libs. **СТАТУС: D1/D2/D4/D5 реализованы
> (X.4.5.1/.2/.4/.5, v.587–599); лакмус ниже актуализирован. D3 —
> остаётся открытым.**
...
> `functions/`. (Реструктуризация: v0.60.541–547,
...
raschet/
  ...
  lib/<id>/            calc-lib (kind:'calc-lib'): suppression-methods,
                       hydraulic-methods
  modules/ help/ dev/  лаунчеры/вспом. (на корне, НЕ модули;
...
- **Есть ли `manifest.json` и запись в `modules.json`?** У `apps/<id>/`
  ...
  `help/`, `dev/` — НЕТ. Это лаунчеры/каталоги/вспом.
...
**Лакмус «CORE-платформа vs CALC-LIB» (ПЕРЕПИСАН 2026-05-17 — D1,
memory:architecture_layers):**

- **CORE = только `js/engine/*`** — ПЛАТФОРМА схемного редактора
  (state/render/serialization/interaction/geometry/history), владеет
  `APP_VERSION`. Универсальная база ВСЕХ типов схем. НЕ запись
  реестра.
- **CALC-LIB = `lib/*-methods/*` + `js/calc/*`** — расчётные МОДУЛИ
  (электрика/гидравлика/ОВиК/газ/пожаротушение). **D1: электрика —
  тоже calc-модуль, НЕ ядро.** Электрика физически перенесена
  `js/methods` → `lib/electrical-methods` (v0.60.593, полный перенос
  без shim). У каждого calc-lib: `manifest.json` + `kind:'calc-lib'`
  + реестр `METHODS/METHOD_LIST/ALL_META`, запись в `modules.json`.
- **`shared/calc`, `shared/calc-modules`** — SHARED-слой расчётных
  хелперов (НЕ запись реестра, по §2 шва).

Тот факт, что `js/engine/recalc.js` импортирует `lib/electrical-methods`
— НЕ делает электрику ядром: **D2** разрешает CORE/любому слою
нативно импортировать calc-lib (boundary-lint: calc-lib — отдельный
класс, не `module`). Правило: математика схемного движка-платформы →
`js/engine` (CORE); ЛЮБАЯ методика расчёта (по стандартам/нормам) →
`lib/<id>-methods` (CALC-LIB, manifest+реестр), нативно используется
ядром и модулями.
